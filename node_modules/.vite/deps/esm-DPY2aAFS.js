...
import { a as __toESM, t as __commonJSMin } from "./rolldown-runtime-Bz_XS7oQ.js";
...
ZodPipeline.create;
//#endregion
//#region node_modules/@ethersproject/logger/lib.esm/_version.js
var version$4 = "logger/5.8.0";
//#endregion
//#region node_modules/@ethersproject/logger/lib.esm/index.js
...
var Logger = class Logger {
	...
	checkNew(target, kind) {
		if (target === Object || target == null) this.throwError("missing new", Logger.errors.MISSING_NEW, { name: kind.name });
	}
	checkAbstract(target, kind) {
		...
		else if (target === Object || target == null) this.throwError("missing new", Logger.errors.MISSING_NEW, { name: kind.name });
	}
	static globalLogger() {
		if (!_globalLogger) _globalLogger = new Logger(version$4);
		return _globalLogger;
	}
	static setCensorship(censorship, permanent) {
		if (!censorship && permanent) this.globalLogger().throwError("cannot permanently disable censorship", Logger.errors.UNSUPPORTED_OPERATION, { operation: "setCensorship" });
...
//#endregion
//#region node_modules/@ethersproject/bytes/lib.esm/index.js
var logger$3 = new Logger("bytes/5.8.0");
...
function arrayify(value, options) {
	...
	if (isHexString(value)) {
		...
		if (hex.length % 2) if (options.hexPad === "left") hex = "0" + hex;
		else if (options.hexPad === "right") hex += "0";
		else logger$3.throwArgumentError("hex data is odd-length", "value", value);
		const result = [];
...
function hexlify(value, options) {
	...
	if (isHexString(value)) {
		if (value.length % 2) if (options.hexPad === "left") value = "0x0" + value.substring(2);
...
function hexZeroPad(value, length) {
	...
	else if (!isHexString(value)) logger$3.throwArgumentError("invalid hex string", "value", value);
	if (value.length > 2 * length + 2) logger$3.throwArgumentError("value out of range", "value", arguments[1]);
	...
	(function(exports$2) {
		...
		*/
		...
		Buffer.prototype.inspect = function inspect() {
			let str = "";
			const max = exports$2.INSPECT_MAX_BYTES;
...
var require_bn = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	(function(module$1, exports$1) {
		...
		try {
			...
			else Buffer = require_dist().Buffer;
		} catch (e) {}
...
//#endregion
//#region node_modules/@ethersproject/bignumber/lib.esm/_version.js
var import_bn = /* @__PURE__ */ __toESM(require_bn());
var version$2 = "bignumber/5.8.0";
//#endregion
//#region node_modules/@ethersproject/bignumber/lib.esm/bignumber.js
/**
*  BigNumber
*
...
var BigNumber = class BigNumber {
	constructor(constructorGuard, hex) {
		if (constructorGuard !== _constructorGuard) logger$2.throwError("cannot call constructor directly; use BigNumber.from", Logger.errors.UNSUPPORTED_OPERATION, { operation: "new (BigNumber)" });
		this._hex = hex;
	...
	div(other) {
		if (BigNumber.from(other).isZero()) throwFault("division-by-zero", "div");
	...
	toBigInt() {
		...
		return logger$2.throwError("this platform does not support BigInt", Logger.errors.UNSUPPORTED_OPERATION, { value: this.toString() });
	}
	toString() {
		if (arguments.length > 0) if (arguments[0] === 10) {
			if (!_warnedToStringRadix) {
				_warnedToStringRadix = true;
				logger$2.warn("BigNumber.toString does not accept any parameters; base-10 is assumed");
			}
		} else if (arguments[0] === 16) logger$2.throwError("BigNumber.toString does not accept any parameters; use bigNumber.toHexString()", Logger.errors.UNEXPECTED_ARGUMENT, {});
		else logger$2.throwError("BigNumber.toString does not accept parameters", Logger.errors.UNEXPECTED_ARGUMENT, {});
	...
	static from(value) {
		...
		if (typeof value === "number") {
			...
			if (value >= MAX_SAFE || value <= -9007199254740991) throwFault("overflow", "BigNumber.from", value);
		...
		if (anyValue) if (anyValue.toHexString) {
			...
			if (typeof hex === "string") {
				if (isHexString(hex) || hex[0] === "-" && isHexString(hex.substring(1))) return BigNumber.from(hex);
			}
		}
...
function toHex(value) {
	...
	if (value[0] === "-") {
		...
		if (value[0] === "-") logger$2.throwArgumentError("invalid hex", "value", value);
		value = toHex(value);
...
*/
//#endregion
//#region node_modules/@ethersproject/keccak256/lib.esm/index.js
...
new Logger("rlp/5.8.0");
//#endregion
//#region node_modules/@ethersproject/address/lib.esm/index.js
var logger = new Logger("address/5.8.0");
function getChecksumAddress(address) {
	if (!isHexString(address, 20)) logger.throwArgumentError("invalid address", "address", address);
...
//#endregion
export { recordType as A, literalType as C, numberType as D, nullType as E, ZodIssueCode as F, unionType as M, unknownType as N, objectType as O, voidType as P, lazyType as S, nativeEnumType as T, custom as _, version$2 as a, functionType as b, arrayify as c, isBytes as d, Logger as f, booleanType as g, bigIntType as h, isBigNumberish as i, stringType as j, promiseType as k, hexZeroPad as l, arrayType as m, getAddress as n, require_bn as o, anyType as p, BigNumber as r, require_dist as s, esm_default as t, hexlify as u, discriminatedUnionType as v, mapType as w, instanceOfType as x, enumType as y };
